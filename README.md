...
> 核心功能已完成：单词 SRS 复习、句子拼写练习、语音评分、学习统计、多词书管理、TTS 朗读、助记图片。所有词书数据与音频资源均为本地文件，无需外部 API。
...
- **TTS 朗读**：单词、例句自动语音播放（浏览器 Speech Synthesis 或本地音频）
- **语音评分**：录音对比 ASR 识别结果，给出发音评分与反馈
- 本地预生成 **12,800+ 条音频文件**（单词 / 例句 / 雅思例句），离线可用

### 助记图片

- **单词助记图**：高考、中考词汇配图辅助联想记忆
- **句子助记图**：雅思对话句子配图辅助情境记忆
- 图片资源本地化，约 **770MB** 图片资源
...
| 中考核心词汇 | `zhongkao` | 单词 | 1,672 |
| 大学英语四级 | `cet4` | 单词 | 3,465 |
| 大学英语六级 | `cet6` | 单词 | 1,761 |
| 雅思核心词汇 | `ielts` | 单词 | 605 |
| 雅思日常对话 | `ielts-sentences` | 句子 | 710 |
| 语言感知句库 | `language-sense-sentences` | 句子 | — |

所有单词词书均包含 **音标、词性、例句、中译** 完整字段。
...
VocabFlow/
├── src/                       # 前端 (React)
│   ├── api/                   # 后端 API 封装 (srs / sentences / user / tts)
│   ├── components/
│   │   ├── layout/            # AppLayout / Sidebar / Topbar
│   │   ├── review/            # FlashCard / GradeButtons / ProgressRing / PronunciationResultPanel
│   │   ├── ui/                # Button / Input / Spinner / Toast
│   │   └── word/              # PronunciationButton
│   ├── data/                  # 词书加载
│   │   └── wordbooks/         # 词书 JSON 数据
│   │       ├── CET4_words.json
│   │       ├── CET6_words.json
│   │       ├── gaokao_words.json
│   │       ├── IELTS_words.json
│   │       ├── IELTS_sentences.json
│   │       ├── language_sense_sentences.json
│   │       └── zhongkao_words.json
│   ├── pages/                 # Today / Review / Words / WordDetail / Sentences / Settings / Stats / WordBookSelection / ...
│   ├── srs/                   # SRS 引擎 (ts-fsrs 封装)
│   ├── stores/                # Zustand store (settings / ui / wordBook / auth)
│   ├── types/                 # TypeScript 类型定义
│   ├── hooks/                 # 应用初始化 / useSwipe
│   └── utils/                 # 常量 / 存储 / 日期 / favorites
...
│       └── srs.ts
├── public/                    # 静态资源
│   ├── audio/                 # 预生成 TTS 音频 (12,800+ 文件)
│   │   ├── word/              # 单词发音
│   │   ├── sentence/          # 句子发音
│   │   └── example/           # 例句发音
│   └── images/                # 助记图片
│       ├── word_mnemonics/    # 单词助记图
│       └── sentence_mnemonics/# 句子助记图
...
2. **FSRS-4.5**：替代传统 SM-2，基于三组件记忆模型预测每张卡片的最佳复习时机，平衡记忆负担与保留率。
3. **纯本地数据**：移除所有外部 API 依赖，词书、音频、图片均为本地资源，可完全离线使用。
4. **句子拼写 UX**：逐词槽输入 + 自动跳转，相比整句输入降低认知负担；渐进式提示（每次揭示一个字母）引导回忆。
5. **预生成资源**：音频（TTS）与图片（助记图）预生成并存放在 `public/`，运行时零外部依赖。

## 已知限制

- 浏览器 Speech Synthesis 的语音质量因系统和浏览器而异（本地音频已弥补）。
- 切换词书会重置 SRS 新词队列（不自动迁移已有卡片状态）。
- 音频与图片资源占用约 1.6GB 本地空间。
