...
export function GradeButtons({ layout, onGrade, disabled }: GradeButtonsProps) {
  ...
  return (
    <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${grades.length}, 1fr)` }}>
      {grades.map((g) => {
        ...
        return (
          <button
            ...
            onClick={() => onGrade(g)}
            className={clsx(
              'rounded-xl py-4 px-3 transition active:scale-95 disabled:opacity-50',
              COLORS[g],
            )}
          >
            <div className="text-lg font-bold">{info.label}</div>
            <div className="text-xs opacity-80 mt-0.5">{info.hint}</div>
            <div className="text-xs opacity-60 mt-1">按 {info.key}</div>
          </button>
        );
      })}
    </div>
